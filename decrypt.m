image= imread('encrypted_image.png');  %讀取圖片
...
%disp(cf)
n=max(cf)    %求n (最大公因數且小於nn)
m=(3*M*N)/(n^2)   %求m(n*n大小矩陣之數量)
...
matrix4=sub_sub_matrices(:, :, :, :);
for ii=1:k
    matrix2=sub_sub_matrices(:, :, :, ii);
    for z=n:-1:1
        for y=n:-1:1
            for x=n:-1:1
                C=int32(mod(A*[x;y;z],n))+1;
                reg=matrix2(C(1),C(2),C(3));
                matrix2(C(1),C(2),C(3))=matrix2(x,y,z);
                matrix2(x,y,z)=reg;
            end
        end
    end
...
k=m/n;
mm=0;
if (k == floor(k))==1    %如果k是整數
    nnm_matrices = zeros(n, n, m);
    for i=1:k
        nnm_matrices(:, :, (i-1)*n+1 : i*n) = matrix4(:, :, :,i);
    end
else    %如果k不是整數
    nnm_matrices = zeros(n, n, m);
    for i=1:floor(k)+1
       if i==floor(k)+1
           nnm_matrices(:, :, (i-1)*n+1:m) = matrix4(:, :, 1:mm,i);
       else
           if i==floor(k)
               mm=m-i*n;    %計算n倍數之多出行數
           end
...
%做bitxor運算
    nnm_matrices(i)=bitxor(BB(1,1), nnm_matrices(i));
    nnm_matrices(i+1)=bitxor(BB(1,2),nnm_matrices(i+1));
    nnm_matrices(i+2)=bitxor(BB(1,3),nnm_matrices(i+2));
    u=u+1;
    if u>Mxa
        u=1;
    end
...
% 將解密後的圖片保存到文件
imwrite(colorImage, 'outpur-decry.png');